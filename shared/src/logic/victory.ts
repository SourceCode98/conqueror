...
import type { EdgeId, VertexId } from '../types/board.js';
import { edgeVertices } from './board.js';
import {
  VICTORY_POINTS_TO_WIN,
  GRAND_ROAD_MIN_LENGTH,
  SUPREME_ARMY_MIN_KNIGHTS,
...
export function calculateLongestRoad(state: GameState, playerId: string): number {
  ...
  function dfs(currentVertex: VertexId, visitedEdges: Set<EdgeId>): number {
    let best = visitedEdges.size;
    for (const edgeId of playerEdges) {
      if (visitedEdges.has(edgeId)) continue;
      const [v1, v2] = edgeVertices(edgeId);
      if (v1 !== currentVertex && v2 !== currentVertex) continue;
      ...
      if (nextBuilding && nextBuilding.playerId !== playerId) continue;
 ...
 */
export function recalculateSpecialCards(state: GameState): PlayerState[] {
  ...
  for (const player of players) {
    if (player.knightsPlayed > maxKnights && player.knightsPlayed >= SUPREME_ARMY_MIN_KNIGHTS) {
    ...
    } else if (!supremeArmyHolder && player.knightsPlayed >= SUPREME_ARMY_MIN_KNIGHTS) {
      player.hasSupremeArmy = true;
      player.victoryPoints += 2;
      supremeArmyHolder = player;
      maxKnights = player.knightsPlayed;
  ...
  let newHolderId: string | null = currentHolder?.id ?? null;
  let maxLen = Math.max(currentHolderLen, GRAND_ROAD_MIN_LENGTH - 1);
 ...
 */
export function calculateTotalVP(player: PlayerState): number {
...
export function checkWinCondition(state: GameState): string | null {
  for (const player of state.players) {
    if (calculateTotalVP(player) >= VICTORY_POINTS_TO_WIN) {
      return player.id;
    }
  }
  return null;
