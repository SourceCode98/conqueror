...
import { adjacentVertices, edgeVertices, hexVertexIds } from './board.js';
import { hasResources } from './resources.js';
import { BUILD_COSTS } from '../constants/board.js';
...
export function canPlaceSettlement(
  ...
  if (!state.board.vertices.includes(vertexId)) return fail('Invalid vertex');

  // No building already there
  if (state.buildings[vertexId]) return fail('Vertex already occupied');

  // Distance rule: no adjacent vertices may have a building
  const boardVertexSet = new Set(state.board.vertices);
  const adj = adjacentVertices(vertexId, boardVertexSet);
  if (adj.some(v => state.buildings[v])) return fail('Too close to another building');
  ...
  if (!isSetupPhase) {
    ...
    if (!hasResources(player.resources, BUILD_COSTS.settlement)) return fail('Insufficient resources');

    // Must have remaining pieces
    if (player.settlementsLeft <= 0) return fail('No settlements remaining');

    // Must have a connecting road
    const hasRoad = state.board.edges.some(eid => {
      const [v1, v2] = edgeVertices(eid);
      if (v1 !== vertexId && v2 !== vertexId) return false;
      return state.roads[eid]?.playerId === playerId;
    });
    if (!hasRoad) return fail('No connecting road');
  } else {
    const player = state.players.find(p => p.id === playerId);
    if (!player) return fail('Player not found');
    if (player.settlementsLeft <= 0) return fail('No settlements remaining');
...
export function canPlaceCity(
  ...
  const player = state.players.find(p => p.id === playerId);
  if (!player) return fail('Player not found');
  if (!hasResources(player.resources, BUILD_COSTS.city)) return fail('Insufficient resources');
  if (player.citiesLeft <= 0) return fail('No cities remaining');
...
export function canPlaceRoad(
  ...
  const player = state.players.find(p => p.id === playerId);
  if (!player) return fail('Player not found');
  if (player.roadsLeft <= 0) return fail('No roads remaining');
  ...
  if (isSetupPhase && setupVertexId) {
    ...
    if (v1 !== setupVertexId && v2 !== setupVertexId) return fail('Road must connect to your settlement');
    return ok();
  }
  ...
  if (!connectsToV1 && !connectsToV2) return fail('No valid connection point');
  return ok();
}

function canRoadConnectAtVertex(
  state: GameState,
  playerId: string,
  edgeId: EdgeId,
  vertexId: VertexId,
